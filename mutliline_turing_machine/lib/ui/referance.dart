...
class Reference extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Также при запуске машины появляется поле с количеством пройденных конфигураций."""),
                      ...
                      text(
                          """Машина Тьюринга, находясь в активном состоянии, производит поиск по всем ее правилам в порядке их следования и при нахождении правила - выполняет шаг. Если же нужного правила не нашлось, то программа выведет уведомление об этом в нижнем правом углу. 

Таким образом, если в состоянии записаны правила “1 2 >” и “1 3 >”, то при нахождении на ленте символа “1” выполнится правило “1 2 >” так как оно находится выше в списке правил чем “1 3 >”."""),
                      subTitle("Управление Машиной Тьюринга"),
                      text(
                          """Машина Тьюринга может работать как  ручном, так и в автоматическом режиме. При ручном режиме для выполнения шага необходимо нажать кнопку “Сделать шаг” на нижней панели инструментов. Автоматический режим подразумевает выполнение шагов машиной без действий со стороны пользователя. Для перехода в автоматический режим нужно нажать на кнопку “Таймер” в нижней панели. Чтобы выйти из автоматического режима необходимо нажать повторно на эту кнопку.

У автоматического режима можно настраивать скорость. Нажав на кнопку изменения скорости рядом с кнопкой “Таймер” появится всплывающий список, в котором можно выбрать желаемую скорость.
...
Чтобы остановить выполнение машины и сбросить текущее состояние нужно нажать на кнопку “Стоп” на нижней панели инструментов."""),
                      ...
                      text(
                          """Каждое правило, вводимое в таблицу правило представляет собой 3 символа в формате “0 1 >”, где “0”- заменяемый символ, “1” - заменяющий символ, а “>” - направление движения головки.
...
Последний столбец в панели правил - столбец переходов. Значение в ячейки - номер состояния, в которое перейдёт машина после выполнения данного правила. По умолчанию переход идёт в это же состояние. 

Доступна возможность быстрого копирования содержимых ячеек таблицы сочетаниями клавиш Ctrl + C, Ctrl + Z.

Для взаимодействия со всеми ячейками правила можно выделить их, нажав Ctrl и кликнув на нужное правило, после чего аналогично с копированием ячейки можно копировать сразу все правило. 

Порядок правил можно изменять. Для этого нужно нажать на 6 точек возле номера правила и потянуть ее на желаемое место. Так же выделив несколько правил можно перемещать их все одновременно."""),
